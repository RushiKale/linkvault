import { getToken, getCollections, getTeams, saveLink, getPageMetadata, login, fetchTags, searchLinks } from './utils/api.js';
import { DASHBOARD_URL } from './utils/config.js';
...
async function init() {
  ...
  try {
    [collections, teams] = await Promise.all([getCollections(), getTeams()]);
    showPopup();
  } catch {
    showLogin();
  }
...
function showLogin() {
  ...
  app.innerHTML = `
    <div class="p-4 space-y-3" style="width: 320px;">
      <a href="${DASHBOARD_URL}/dashboard" target="_blank" style="text-decoration:none;color:inherit">
      ...
      <a href="${DASHBOARD_URL}/login" target="_blank" class="text-sm text-center block text-primary">
        Create an account
      </a>
    </div>
  `;
...
function showPopup() {
  getPageMetadata().then((meta) => {
    ...
    app.innerHTML = `
      <div style="width: 360px; padding: 16px;">
        <div class="flex items-center gap-2 mb-4">
          <a href="${DASHBOARD_URL}/dashboard" target="_blank" class="flex items-center gap-2" style="text-decoration:none;color:inherit">
        ...
        <div id="view-save" class="hidden">
          ...
          ${hasTeams ? `
          <div class="mb-3">
            <label class="text-xs font-medium text-muted mb-1 block">Team</label>
            <select id="team-select" class="input">
              <option value="">Personal</option>
              ${teams.filter((t: any) => t.role !== 'PENDING').map((t: any) => `<option value="${t.id}">${t.name}</option>`).join('')}
            </select>
          </div>
          ` : ''}
    ...
    // Team selector: filter collections
    ...
    if (teamSelect) {
      function filterCollections() {
        ...
        const filtered = teamId
          ? collections.filter((c: any) => c.teamId === teamId)
          : collections.filter((c: any) => !c.teamId);
        collectionSelect.innerHTML = filtered.length > 0
          ? filtered.map((c: any) => `<option value="${c.id}">${c.name}</option>`).join('')
          : '<option value="">No collections</option>';
      }
      teamSelect.addEventListener('change', filterCollections);
      filterCollections();
    }
    }
      ...
      searchResults.innerHTML = links
        .map((link: any) => {
          ...
          return `<div class="search-result" data-url="${escapeHtml(link.url)}">
            ...
            <span class="search-result-favicon-placeholder" style="display:none;background:rgba(239,68,68,0.12);color:#ef4444">${escapeHtml(placeholder)}</span>
